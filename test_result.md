...
agent_communication:
  ...
  - agent: "main"
    message: "Fixed intermittent 520 timeout error on AI chat page. Implemented comprehensive timeout handling with asyncio.wait_for() wrapper (60-second timeout), detailed logging for debugging, and specific error handling for timeouts. Backend now returns 504 Gateway Timeout instead of 520, preventing proxy issues. Frontend updated to display user-friendly error messages. Changes made to /app/backend/main.py (added logging, asyncio timeout wrapper) and /app/app/ai/page.js (improved error handling). Ready for testing to verify timeout handling works correctly."